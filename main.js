...
import { spawnFauna, updateFauna } from './scripts/fauna.js'; // Import fauna spawning and update functions
...
import { PerformanceMonitor } from './scripts/performanceMonitor.js';
...
let allFauna = [];    // Container for fauna spawned outside town
...
// --- flora interaction constants ---
const FLORA_NAMES = new Set([
    'AgavePlant', 'Bushes3', 'GlowingBlueMushroom', 'LowPolyPlant', 'Mushrooms',
    'MutaPlant', 'MutaPlant2', 'MutaPlant3', 'PineForest', 'Potato', 'TallCornPlantHigh'
]);
...
async function preloadEssentialModels() {
    ...
    let essentialModelUrls = await fetch(new URL('assets/manifest.json', document.baseURI))
        .then(r => r.json())
        .then(urls => urls.map(u => {
            let p = u.startsWith('/') ? u.substring(1) : u;
            // Fix dancer paths: remove 'static/' for diamond_dancers
            p = p.replace(/^assets\/static\/diamond_dancers/, 'assets/diamond_dancers');
            return p;
        }));
...
async function init() {
        ...
        for (const createFlora of floraCreators) {
            for (let i = 0; i < 20; i++) {
                ...
                try {
                    ...
                    const plant = await createFlora(scene, x, z, collidableObjects);
                    if (plant) {
    ...
    // Spawn wild fauna (bears, chickens, cows, foxes)
    try {
        const faunaCreatures = await spawnFauna(scene, (progress) => {
            // Progress tracking for loading screen
            if (progress && progress < 1) {
                updateStatus(0.9 + (progress * 0.1), `Loading fauna... ${Math.floor(progress * 100)}%`);
            }
        }, [player.position]);
        
        if (faunaCreatures && faunaCreatures.length > 0) {
            faunaCreatures.forEach(fauna => {
                ...
                allCreatures.push(fauna); // Add to creatures array for updates
                allFauna.push(fauna);   // track fauna for updates
            });
        }
    } catch (error) {
...
function interactWithFlora() {
    if (!player) return false;
    const playerPos = player.position;
    let interacted = false;
    // Traverse scene children once; for large scenes could optimize but acceptable
    scene.traverse(obj => {
        if (interacted) return; // already handled
        if (!obj.name || !FLORA_NAMES.has(obj.name)) return;
        const dist = playerPos.distanceTo(obj.position);
        if (dist <= INTERACTION_DISTANCE) {
            interacted = true;
            showPopup({
                title: obj.name,
                html: `You examine the ${obj.name}.`,
...
async function interact() {
    ...
    if (!humanoidHit) {
        ...
        if (!clubHit) {
            ...
            if (!tavernHit) {
                const floraHit = interactWithFlora();
                if (!floraHit) {
...
function animate() {
    ...
    if (now - lastHeartbeat > 5000) {
        ...
        if (perfSummary.degraded) {
            console.warn('Performance degraded:', perfSummary);
        }
    ...
    // Update Fauna (bears, cows, foxes, etc.) - chickens are handled separately
    const faunaInView = allFauna.filter(fauna => 
        _frustum.containsPoint(fauna.position) && 
        fauna.userData.species !== 'chicken'  // Skip chickens
    );
    faunaInView.forEach(fauna => {
        // Advance fauna animations
        if (fauna.userData.mixer) fauna.userData.mixer.update(deltaTime * animFactor);
        updateFauna(fauna, deltaTime * moveFactor, collidableObjects, scene);
    });
    
    // Update Chickens with collision (separate from other fauna)
    ...
    allCreatures.forEach(cre => {
        ...
        // Handle fauna type creatures (bears, cows, foxes) - chickens are handled separately
        if (cre.userData.type === 'fauna' && cre.name !== 'RadChicken') {
            updateFauna(cre, deltaTime * moveFactor, obstaclesCre, scene);
        }
        // Handle other creature types
        else if (cre.name === 'AlienArmored') {
    ...
    performanceMonitor.update(deltaTime, scene, {
...
function playRandomBgm() {
    ...
    lastBgmIndex = idx;
    const trackUrl = new URL(bgmTracks[idx], document.baseURI).href;
...
onStartGame(async () => {
    hideStatus();
    // Don't call init() again as it's already called automatically at load time
...
async function spawnAssetUI() {
    ...
    const manifest = await fetch(new URL('assets/manifest.json', document.baseURI)).then(r=>r.json());
    const groups = {};
...
window.addEventListener('keydown', event => {
  if (event.code === 'KeyU' && player && camera) {
    ...
    if (Math.abs(player.position.x) <= cs.x/2 && Math.abs(player.position.z) <= cs.z/2) {
      ...
      const baseOff = config.camera.offset; // Get base offset from config
