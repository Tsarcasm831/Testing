...
- **Static land pages:** `land.html` is a template for individual land detail pages. `generate-land-pages.js` can be run with Node (`node generate-land-pages.js`) to generate static `LandXX.html`/`IslandXX.html` pages from default land data. Per-land HTML files (e.g., `Land01.html`) are prebuilt and can be opened directly.
