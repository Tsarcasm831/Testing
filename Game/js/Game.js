...
export class Game {
    ...
    async init(shouldPreloadAssets = true) {
        function createOfflineRoom() {
            const stubCollection = () => ({
                subscribe: () => {},
                getList: () => [],
                create: async () => {},
                update: async () => {},
                filter: () => stubCollection()
            });
            return {
                clientId: 'offline',
                peers: {},
                ...
                send: () => {},
                updatePresence: () => {},
                updateRoomState(state) { this.roomState = { ...this.roomState, ...state }; },
                collection: () => stubCollection()
            };
        }

        const WebsimSocketClass = globalThis.WebsimSocket;
        if (WebsimSocketClass) {
            this.room = new WebsimSocketClass();
            await this.room.initialize();
        } else {
            this.room = createOfflineRoom();
        }
