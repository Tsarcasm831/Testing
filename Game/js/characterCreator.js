...
Be creative in interpreting the user's description, but stay true to the details provided. The output should be a JSON object with each body part as a key and its detailed description as the value.`;
            ...
            if (!window.websim?.chat?.completions?.create) {
                throw new Error('Character generation unavailable');
            }
            ...
            const conversionCompletion = await window.websim.chat.completions.create({ messages: conversionMessages, json: true });
