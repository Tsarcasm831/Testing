...
export function drawGhost(isRoad=false,isWall=false){
  ...
  const el = isWall || kind==='wall'
    ...
    ? mkEl('polyline',{class:(kind==='river'?'river':(kind==='grass'?'grass':(kind==='forest'?'forest':'mountain')))+' ghost',points:pts,strokeWidth:(kind==='grass'?300:(kind==='forest'?10:(kind==='mountain'?10:3)))})
...
export function finishDrawing(){
  ...
  if(!isRoad && !isRiver && !isWall && !isGrass && !isForest && !isMountain){
    ...
    MODEL.grass.push({id:'grass-'+(MODEL.grass.length+1),points:state.drawing.points,width:300});
  }else if(isForest){
