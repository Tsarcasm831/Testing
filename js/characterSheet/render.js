...
export function generateCharacterSheetHTML(charData){
    const weaponsData = charData.weapons && charData.weapons.weapons ? charData.weapons.weapons : {};
    const weapons = Object.entries(weaponsData).flatMap(([category, weaponList]) => 
        weaponList.map(weapon => ({ ...weapon, category }))
    );
