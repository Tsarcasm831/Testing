...
(function(ns) {
    ...
    ns.ui.showShopTab = () => {
        if (!DOMElements.navLinksContainer || document.querySelector('[data-page="shop"]')) return;
        const shopLink = document.createElement('a');
        shopLink.href = '/?page=shop';
        shopLink.className = 'nav-link';
        shopLink.dataset.page = 'shop';
        shopLink.textContent = 'SHOP';
        DOMElements.navLinksContainer.appendChild(shopLink);
    };
    
    ns.ui.hideShopTab = () => {
        const shopLink = DOMElements.navLinksContainer?.querySelector('[data-page="shop"]');
        if (shopLink) {
            shopLink.remove();
        }
    };
